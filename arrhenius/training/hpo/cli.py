...
def build_parser():
    ...
    p.add_argument("--rad-source", choices=["path", "rad", "default"], default="path",
    ...
    p.add_argument("--metrics-db", default="metrics.sqlite",
               help="SQLite file to store trial metrics/config/scaler stats/split indices.")
    p.add_argument("--save-raw-scalers", action="store_true",
               help="Also persist raw scaler vectors (may be large).")
    p.add_argument("--arr-layer", choices=["on", "off"], default="on",
                   help="Enable or disable Arrhenius layer in the model (default: on).")
    p.add_argument("--arr-supervision", choices=["on", "off"], default="on",
    ...
    p.add_argument("--final-test-frac", type=float, default=0.15,
    ...
    p.add_argument("--final-metrics-db", default="final_metrics.sqlite",
                   help="SQLite file where the reproducibility-focused evaluations are stored.")
    p.add_argument("--final-study-name", default="arrhenius_final",
    ...
    p.add_argument("--splitter", choices=["kstone", "random"], default="kstone",
    ...
    p.add_argument("--reset-split-exports", action="store_true",
                   help="Remove any existing split export CSV/SQLite before writing new results.")
    p.add_argument("--record-final-preds", action="store_true", default=True,
                   help="Record per-sample predictions (scaled and unscaled) during the final top-k CV sweep.")
    p.add_argument("--no-record-final-preds", action="store_false", dest="record_final_preds",
