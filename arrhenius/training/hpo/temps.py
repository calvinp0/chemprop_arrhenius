...
def build_temps(cfg, args, yaml_space=None, default=(300.0, 3100.0, 100.0), include_max=False):
    ...
    if tstep <= 0:
        raise ValueError(f"temp_step must be > 0 (got {tstep})")
    if tmax <= tmin:
        raise ValueError(f"temp_max ({tmax}) must be > temp_min ({tmin})")

    temps = np.arange(
        tmin, tmax, tstep, dtype=float
    )  # endpoint excluded (matches your current call)
