...
def apply_yaml_space(
    trial: Trial, cfg: Dict[str, Any], space: Dict[str, Any], chosen_family
) -> Dict[str, Any]:
    c = deepcopy(cfg)

    # 1) flat keys (skip 'global_feats')
    for key, spec in space.items():
        if key in {"global_feats", "rank_metric_default", "rank_metric"} or str(key).startswith(
            "_"
        ):
            continue
