...
class ArrheniusCheckpointIOMixin:
    ...
    def configure_optimizers(self) -> dict[str, object]:
        ...
        if self.trainer.max_epochs == -1:
            logger.warning(
                "For infinite training, cooldown epochs are set to 100x warmup epochs."
            )
    ...
    def set_scalers(
        ...
        *,
        y_scaler: Any = None,
        vf_scaler_shared: Any = None,
        xd_scaler_shared: Any = None,
    ) -> None:
    ...
    def sample_stratified_indices(self, num_bins: int = 12, samples_per_bin: int = 1) -> Tensor:
        ...
        bin_edges = torch.linspace(T.min().to(T.device), T.max().to(T.device), steps=num_bins + 1).to(T.device)
        sampled_indices = []
        ...
        for i in range(num_bins):
            ...
            if len(indices_in_bin) > 0:
                ...
                if g is None:
                    ...
                    self._strat_rng = g
                selected = indices_in_bin[torch.randperm(len(indices_in_bin), generator=g)[:samples_per_bin]]
